...
import { Plus, X, AlertCircle, Home, MapPin, Users, Bed, DollarSign, Image as ImageIcon, CheckCircle, Save } from 'lucide-react';
...
export default function EditUnitForm({ unitId }: { unitId: number }) {
  ...
  useEffect(() => {
    if (unitId > 0) {
      ...
      fetch(`http://localhost:5050/api/admin/accommodations/${unitId}`, {
        method: 'GET',
        credentials: 'include',
      })
      .then(async (res) => {
      ...
      .then(data => {
        setFormData({
          ...
          status: (data.status_unit as UnitStatus) || 'available',
          price: data.price !== undefined ? String(data.price) : '',
          rooms: data.jumlah_kamar !== undefined ? String(data.jumlah_kamar) : '',
          location: data.alamat || '',
          capacity: data.capacity !== undefined ? String(data.capacity) : '',
          description: data.description || '',
          amenities: data.fasilitas || [],
      ...
      .catch(err => {
         setError('Data Tidak Ditemukan: ' + err.message);
         setLoadingInitial(false);
      });
    } else {
      setLoadingInitial(false);
  ...
  const validateForm = () => {
    if (!formData.name.trim()) { setError('Nama unit harus diisi'); return false; }
    if (!formData.category.trim()) { setError('Kategori harus diisi'); return false; }
    if (!formData.price.trim()) { setError('Harga harus diisi'); return false; }
    if (!formData.rooms.trim()) { setError('Jumlah kamar harus diisi'); return false; }
    if (!formData.location.trim()) { setError('Lokasi harus diisi'); return false; }
    if (!formData.capacity.trim()) { setError('Kapasitas harus diisi'); return false; }
    return true;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5050/api/admin/accommodations/${unitId}`, {
        method: 'PUT',
        credentials: 'include',
        body: sendData,
      });
      if (!res.ok) {
        ...
        throw new Error(errorData.error || 'Gagal memperbarui unit');
      }
      setSuccess('Unit berhasil diperbarui!');
      setTimeout(() => { setSuccess(''); window.location.href = '/admin'; }, 1200);
    } catch (err) {
  ...
  if (error && error.includes('Data Tidak Ditemukan')) {
    return (
      <div className="w-full max-w-4xl mx-auto">
        <div className="bg-white rounded-xl shadow-md border border-gray-200 p-8">
          <div className="text-center py-12">
            <AlertCircle className="w-16 h-16 text-red-500 mx-auto mb-4" />
            <p className="text-lg font-semibold text-black mb-2">Data Tidak Ditemukan</p>
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-black mb-2">Edit Unit</h1>
        <p className="text-gray-600">Perbarui informasi unit properti Anda dengan detail</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="flex items-center justify-center w-full">
              <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 transition-all">
                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                  ...
                  <p className="text-sm text-gray-500 underline">Add more photos</p>
                  <p className="text-xs text-gray-400 mt-1">Select files to upload (replaces current if needed by backend)</p>
                </div>
                <input 
                  type="file" 
                  name="images"
        ...
        <div className="flex gap-4 pt-4">
          <button
            type="submit"
            disabled={loading}
            className="flex items-center justify-center gap-2 px-12 py-3 bg-gray-900 text-white rounded-xl hover:bg-gray-800 transition-all shadow-md hover:shadow-lg"
          >
            {loading ? (
              <>
